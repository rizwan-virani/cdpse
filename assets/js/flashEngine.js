/* ============================================================================
   CDPSE  ::  flashEngine.js  —  Rapid Recall Flashcards
   ...
   - Cards live in lazy-loaded per-domain modules (assets/js/content/flashN.js),
     each populating CDPSE.flash[N] with 100 cards { id, d, t, obj, front, back }.
   - Scheduling state persists to localStorage (cdpse.flash.v1) keyed by card id,
...
(function () {
  ...
  /* Domain id list derived from the single source of truth so this engine works
     for any domain count (CDPSE has 4). Never hardcode 5 / 500 / [1..5] here. */
  function DOMS() { return (S.domainMeta || []).map(function (d) { return d.id; }); }
  function TOTAL_CARDS() { return DOMS().length * DECK_SIZE; }

  function ver() { return window.CDPSE_VER ? "?v=" + window.CDPSE_VER : ""; }
